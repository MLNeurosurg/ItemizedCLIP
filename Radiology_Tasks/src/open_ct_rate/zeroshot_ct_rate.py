"""
Adapted from HLIP repository: https://github.com/Zch0414/hlip/blob/master/src/hlip_test/zeroshot_ct_rate.py
...
from open_ct_rate.zeroshot_metadata_ct_rate import CLASSNAMES, ORGANS

import torch.nn.functional as F

INPUT_CSV = '../open_ct_rate/valid_labels.csv'
...
def get_args_parser():
    ...
    parser.add_argument('--resume', default='/scratch/tocho_root/tocho99/yiweilyu/logs/2025_09_07-23_54_37-model_CTRATE_HLIP_BiomedBERT_flair-lr_0.0001-b_64-j_8-p_amp/checkpoints/epoch_68.pt', type=str)

    parser.add_argument('--data-root', default='/data/ct_rate/')
    parser.add_argument('--zeroshot-ct-rate', default='../open_ct_rate/valid_labels.csv', type=str)
...
# Create dataset for zero-shot evaluation
def get_data(args, preprocess_fn=None):
    ...
    dataset = ZeroShotDataset(
        args.data_root, INPUT_CSV, ["-1150", "350", "crop"],
        preprocess_fn
    )
    ...
    with torch.inference_mode():
        ...
        for batch in tqdm(dataloader, total=len(dataloader)):
            ...
            with autocast():
                ...
                attn_out = output['visual_proj'](textembs.unsqueeze(0).expand(len(image_features),-1,-1),image_features,image_features) # BxCx512
                ...
                for i,key in enumerate(CLASSNAMES):
                    for jj in range(len(image_features)):
                        logits[key].append([sims[jj,i+j*len(CLASSNAMES)].cpu().float().item() for j in range(numtextperclass)])
            for idx in range(target.shape[1]):
                for jj in range(len(image_features)):
                    labels[CLASSNAMES[idx]].append(target[jj, idx].cpu().float().item())
        ...
        for j in range(numtextperclass):
            ...
            for key in CLASSNAMES:
                threshold = find_threshold(np.array(labels[key]), np.array(logits[key])[:,j])
...
def main(args):
    ...
    # zero shot
    results, alltexts = zero_shot(model, tokenizer, data, args)
    ...
    for i,key in enumerate(results):
        ...
        for subkey in results[key]:
            if 'auc' in subkey:
                print(key+' '+subkey+': '+str(results[key][subkey]))
                if results[key][subkey] > bestauc:
                    bestauc = results[key][subkey]
                    bestsubkey = subkey
        optims[key] = {}
        for subkey in results[key]:
            if bestsubkey[-1] in subkey:
                optims[key][subkey[:-1]] = results[key][subkey]
            optims[key]['text'] = alltexts[i+int(bestsubkey[-1])*len(CLASSNAMES)]
    means = {}
    for key in optims:
        for subkey in optims[key]:
            if subkey == 'text':
                continue
            if subkey not in means:
                means[subkey] = 0.0
            means[subkey] += optims[key][subkey] / len(CLASSNAMES)
