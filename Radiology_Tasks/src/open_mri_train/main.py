"""
Main training script for brain MRI, adapted from HLIP repository: 
"""
...
from open_mri.factory import create_loss as custom_create_loss

from open_mri_train.data import get_data
from open_mri_train.params import parse_args
from open_mri_train.train import train_one_epoch, evaluate
from open_clip import trace_model
from open_mri.factory import create_model_and_transforms,  get_tokenizer
from open_clip.factory import create_loss as open_clip_create_loss
...
def main(args):
    ...
    model, preprocess_train, preprocess_val = create_model_and_transforms(
        ...
        my_custom_model = args.itemizedclip_text or args.use_itemizedclip_loss,
        **model_kwargs,
    )
    ...
    # initialize datasets
    tokenizer = get_tokenizer(args.model, cache_dir=args.cache_dir, context_length=args.context_length, trust_remote_code=True)
    data = get_data(
            args,
            tokenizer=tokenizer,
        )
    ...
    if args.custom_clip_loss or args.correct_false_negatives or args.use_itemizedclip_loss:
        loss = custom_create_loss(args)
        logging.info('Use custom clip loss.')
    else:
        loss = open_clip_create_loss(args)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if completed_epoch % args.save_and_eval_every == 0:

            if any(v in data for v in ('val', 'imagenet-val', 'imagenet-v2', 'zeroshot-ct-rate')):
                evaluate(model, data, completed_epoch, args, tb_writer=writer, tokenizer=tokenizer, loss=loss)
                if 'val2' in data:
                    evaluate(model, data, completed_epoch, args, tb_writer=writer, tokenizer=tokenizer, val2=True, loss=loss)
