"""
This script is directly adapted from https://github.com/Zch0414/hlip/blob/master/src/hlip/pos_embed.py
"""
...
def resample_series_posemb(series_posemb, num_series, is_train=True):
    """resample series position embedding.
    """
    series_posemb = series_posemb.float()
    max_num_series = series_posemb.shape[1]

    # interpolate
    if max_num_series < num_series:
        assert not is_train
        series_posemb = F.interpolate(series_posemb.permute(0, 2, 1), size=num_series, mode='linear').permute(0, 2, 1)
        return series_posemb
    
    # sample
    if num_series <= max_num_series:
        if is_train:
            perm = torch.randperm(max_num_series)[:num_series]
        else:
            perm = torch.arange(num_series)
        series_posemb = series_posemb[:, perm, :]
        return series_posemb


def resample_spatial_posemb(spatial_posemb, new_size, old_size):
    # new_size and old_size should be provided with the same shape: [d, h, w]
    # d: through-place dimension, h and w: in-place dimension.
    spatial_posemb = spatial_posemb.float()
    if new_size == old_size:
        return spatial_posemb

    # interpolate
    if old_size[0] != new_size[0] or old_size[1] != new_size[1] or old_size[2] != new_size[2]:
        spatial_posemb = F.interpolate(spatial_posemb.permute(0, 4, 1, 2, 3), size=(new_size[0], new_size[1], new_size[2]), mode='trilinear').permute(0, 2, 3, 4, 1)

    return spatial_posemb


def mri_pos_embed(max_num_series, grid_size, embed_dim, pretrained_posemb=None): 
    """
    ...
    Return:
        ...
        series_posemb: A tensor of shape [1, max_num_series, embed_dim]
    """
    if pretrained_posemb is not None: 
        ...
        pretrained_posemb = resample_abs_pos_embed(
            pretrained_posemb,
        ...
        series_posemb = get_1d_sincos_pos_embed(
            embed_dim=embed_dim,
            sequence_len=max_num_series,
            cls_token=False
        ) # [n, embed_dim]
        series_posemb = torch.from_numpy(series_posemb).float()[None, ...]
    else: 
        # build mri position embedding from scratch: study_posemb (1d) + series_posemb (3d)
        ...
        series_posemb = torch.from_numpy(series_posemb).float()[None, ...]
    return spatial_posemb, series_posemb
