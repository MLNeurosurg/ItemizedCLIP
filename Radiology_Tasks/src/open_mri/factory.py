"""
Factory adapted from open-clip repository: https://github.com/mlfoundations/open_clip/blob/main/src/open_clip/factory.py

The code integrates additional modifications and extensions to support HLIP and ItemizedCLIP models.
"""

import sys
sys.path.append('.')

from open_mri.loss import CustomClipLoss, CustomSigLipLoss
from open_mri.itemizedclip_loss import ItemizedCLIPLoss
...
from .my_model import ItemizedCLIP
...
def create_model(
    ...
    if custom_text:
        if "multimodal_cfg" in model_cfg:
            ...
            if my_custom_model:
                model = ItemizedCLIP(**model_cfg, cast_dtype=cast_dtype)
            else:
...
def create_loss(args):
    if args.use_itemizedclip_loss:
        return ItemizedCLIPLoss(rank=args.rank,
                            world_size=args.world_size,
                            num_cap_per_img=args.num_sampled_captions,
                            added_mps_loss=True, iis_loss = args.iis_loss, mps_fac = args.mps_fac,  maskposrate = args.ila_mask_rate, ila_fac = args.ila_fac,
                            no_normal_check = args.no_normal_check, comp_upfac=args.comp_upfac,
                            key_token_alignment_loss=args.key_token_alignment_loss,key_token_thresh=args.key_token_thresh)


    if args.siglip:
        assert not args.horovod, "Horovod not currently supported for SigLip"
        return CustomSigLipLoss(
            ...
            local_global_text = args.flair_text
        )
