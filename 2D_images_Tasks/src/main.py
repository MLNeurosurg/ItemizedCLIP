...
Truncated version of the main.py from outer folder
"""
...
def main(args):
    ...
    if args.resume is not None:
        ...
        if 'epoch' in checkpoint:
            ...
            if 'image_post2.proj' in sd:
                #print(sd['image_post.proj'])
                #print(sd['image_post2.proj'])
                postproj = sd.pop('image_post2.proj')  # remove old key that no longer exists
                sd['image_post.proj'] = postproj  # add it back with the new key name
    ...
    if args.wandb and is_master(args):
        ...
        wandb.init(
            ...
            config=vars(args),
            dir = os.path.abspath(args.wandb_dir)
        )
        if args.debug:
            wandb.watch(model, log='all')
        wandb.save(os.path.abspath(params_file))
    ...
    if 'train' not in data:
        ...
        if args.classification_dataset is not None:
            from flair.classification import classify
            classify(model, data['rsicd'].dataloader, args, tokenizer)
        # Evaluate.
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if any(v in data for v in (
                'val',  'retrieval_coco', 'retrieval_flickr')):
            if args.world_size < 2 or get_rank() == 0:
